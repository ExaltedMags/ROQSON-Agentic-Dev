...
const SALES_COL_WIDTHS = {
    ...
    "sales-col-address":     "450px", // Extremely wide to prevent truncation
...
// ── Robust column styling function (Exact clone of Order Form) ───────────────
function style_all_columns(page) {
    ...
    page.querySelectorAll(".list-row-container .list-row").forEach(row => {
        ...
        // Step 3: Move ID col after subject
        ...
        // Hide empty dashes inside checkboxes
        if (subCol) {
...
frappe.listview_settings["Sales"] = {
    add_fields: ["status", "customer_link", "address", "grand_total", "order_ref", "creation_date", "fulfillment_type", "name"],
...
print("Applied exact data-filter column logic from Order Form.")
