...
# Helper to check if a Sales record is already on this specific Trip Ticket
def is_already_on_this_trip(sales_name):
    if doc.is_new():
        return False
    return frappe.db.exists("Trip Ticket Table", {"parent": doc.name, "sales_no": sales_name})

# Updated eligibility check
# Always allow Pending. Allow others only if already linked to this trip.
status = first_sales.get("status")
if status != "Pending" and not is_already_on_this_trip(first_sales_name):
    frappe.throw(
...
for r in rows:
    ...
    # Validate status
    if status != "Pending" and not is_already_on_this_trip(sales_name):
        frappe.throw(
            "Sales record " + str(sales_name) + " is not eligible (status must be Pending, got " + str(status) + ")."
        )
"""

print("Refining Enforce Eligibility to support full delivery lifecycle...")
