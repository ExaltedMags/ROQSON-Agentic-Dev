...
// Sales: Master Script (Combined Form & List logic)
// Separated to keep list view clean and ensure roles work correctly.
...
frappe.ui.form.on('Sales', {
    ...
    refresh(frm) {
        // Pick-up button for Warehouse/Admin
...
# Fetch the existing script ID to update properly
client_scripts = roqson.get_scripts_for_doctype("Sales")['client']
target_script = next((s for s in client_scripts if s['name'] == 'Sales: Paid Validation'), None)

if target_script:
    roqson.update_doc("Client Script", target_script['name'], {"script": new_sales_list_script})
    print(f"Updated {target_script['name']}")
else:
    # Fallback to creating if name doesn't match
    doc = {"doctype": "Client Script", "name": "Sales: Master Script", "dt": "Sales", "script": new_sales_list_script, "enabled": 1}
    requests.post("https://roqson-industrial-sales.s.frappe.cloud/api/resource/Client Script", json=doc, headers=headers)
    print("Created Sales: Master Script")
