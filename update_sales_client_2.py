...
# We will completely replace the script since we are rearranging the setup and refresh logic.
new_script = """// DocType: Sales
// Logic for table calculations and footer rendering to mimic Order Form

frappe.ui.form.on('Sales', {
    setup(frm) {
        const is_sales = frappe.user_roles.includes('Sales') || frappe.user_roles.includes('Sales Manager');
        
        if (frm.fields_dict.items && frm.fields_dict.items.grid) {
            let grid = frm.fields_dict.items.grid;
            
            // Set required fields for items table
            grid.update_docfield_property('item', 'reqd', 1);
            grid.update_docfield_property('qty', 'reqd', 1);
            
            if (is_sales) {
                grid.update_docfield_property('warehouse', 'reqd', 1);
            }
        }
    },
    ...
    validate(frm) {
        ...
        (frm.doc.items || []).forEach((row, i) => {
            if (!row.item || !row.qty) {
                frappe.throw(__('Row {0}: Item and Qty are required.', [i + 1]));
            }
            if (is_sales && !row.warehouse) {
                frappe.throw(__('Row {0}: Warehouse is required for Sales role.', [i + 1]));
            }
        });

        // Additional safeguard to prevent saving if someone managed to delete rows
        if (frm.doc.__unsaved === 1 && frm.doc.items) {
             // Since we can't easily check old vs new items count in client script validate, 
             // enforcing cannot_delete_rows and cannot_add_rows is generally enough.
             pass;
        }
