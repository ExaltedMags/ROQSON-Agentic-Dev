...
frappe.listview_settings['Sales'] = {
    ...
    // Removed get_indicator so it doesn't conflict with our custom formatter
    ...
    onload(listview) {
        ...
        // Add the bundle action to the bulk actions menu
    ...
    add_bundle_actions(listview) {
        ...
        if (has_role) {
            // Add to the bulk Actions menu (which appears when checkboxes are selected)
            listview.page.add_actions_menu_item('Create Trip Ticket', () => {
                const selected = listview.get_checked_items();
                if (!selected.length) {
                    frappe.msgprint('Select at least one Sales record.');
                    return;
                }
                if (selected.some(d => d.status !== 'Pending')) {
                    frappe.msgprint('Only Pending Sales records allowed.');
                    return;
                }
                if (selected.some(d => d.fulfillment_type === 'Pick-up')) {
                    frappe.msgprint('Pick-up orders skip Trip Tickets.');
                    return;
                }
            ...
            // Also add a fallback primary action button just in case the bulk menu doesn't behave as expected
            listview.page.add_inner_button('Create Trip Ticket', () => {
                const selected = listview.get_checked_items();
                if (!selected.length) {
                    frappe.msgprint('Please select at least one Sales record using the checkboxes.');
                    return;
                }
                if (selected.some(d => d.status !== 'Pending')) {
                    frappe.msgprint('Only Pending Sales records allowed.');
